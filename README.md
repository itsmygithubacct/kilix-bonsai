...
The three task interfaces and the text launcher draw through the shared panel
chrome from `kilix-tui-utils` when it is installed, and fall back to a plain
title and rule when it is not. The optional pixel launcher uses that package's
full desktop renderer. A bare checkout over `ssh` still runs, and every screen
still renders to plain text for the tests.
...
The text launcher fills its spare width with the Kilix mascot: a flame kitten
