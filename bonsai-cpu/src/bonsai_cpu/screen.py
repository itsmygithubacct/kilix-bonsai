...
This is deliberately the same shape and the same keymap as the shared core
the Kilix terminal utilities use — a user who knows one of those tools can
drive this one without reading anything. It is a small copy rather than an
import because this repository must stand alone: a chat interface that needs
a second checkout present to draw a list would be the wrong trade. If the
copies ever need to diverge, that is the signal to move this file into the
shared core instead.

Two additions over the shared shape, both made so a streaming chat can be
drawn honestly: `write` carries a curses attribute (bold, dim) through to
the surface, and the loop can be ended by a background thread setting
`state.finished` — a delegating tool needs to leave the screen without
waiting for a keypress.

`render(surface, state)` is all a screen has to supply. Rendering into
`TextSurface` instead of a terminal is what makes every screen here
assertable as plain text, with no pty and no display.
"""
...
class TextSurface:
    ...
    Attributes are accepted and dropped: headless assertions are about the
    words on the screen, and styling promises are asserted where the spans
    are made, not here.
    """
...
def write(surface: Surface, y: int, x: int, text: str, attr: int = 0) -> None:
    ...
    screen's problem. The attribute travels with the text; clipping math is
    unaffected by it.
    """
